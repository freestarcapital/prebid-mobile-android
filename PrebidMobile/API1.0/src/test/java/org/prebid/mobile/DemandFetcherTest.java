...
public class DemandFetcherTest extends BaseSetup {
    ...
    public void testBaseConditions() throws Exception {
        if (successfulMockServerStarted) {
            ...
            AdUnit adUnit = new BannerAdUnit("Ad1", null, 320, 50);
            DemandFetcher demandFetcher = new DemandFetcher(adUnit, request);
            demandFetcher.setPeriodMillis(0);
            HashSet<AdSize> sizes = new HashSet<>();
            sizes.add(new AdSize(300, 250));
            ArrayList<String> keywords = new ArrayList<>();
            RequestParams requestParams = new RequestParams("12345", AdType.BANNER, sizes, keywords, new ArrayList<AdNetwork>(), null);
    ...
    public void testSingleRequestOneBidResponseForMoPubAdObject() throws Exception {
        if (successfulMockServerStarted) {
            ...
            AdUnit adUnit = new BannerAdUnit("Ad1", 320, 50);
            DemandFetcher demandFetcher = new DemandFetcher(adUnit, adView);
            PrebidMobile.setTimeoutMillis(Integer.MAX_VALUE);
            demandFetcher.setPeriodMillis(0);
            HashSet<AdSize> sizes = new HashSet<>();
            sizes.add(new AdSize(300, 250));
            ArrayList<String> keywords = new ArrayList<>();
            RequestParams requestParams = new RequestParams("12345", AdType.BANNER, sizes, keywords, new ArrayList<AdNetwork>());
            demandFetcher.setRequestParams(requestParams);
            OnCompleteListener mockListener = mock(OnCompleteListener.class);
            demandFetcher.setListener(mockListener);
            assertEquals(DemandFetcher.STATE.STOPPED, FieldUtils.readField(demandFetcher, "state", true));
